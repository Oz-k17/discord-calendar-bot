 ...
 * | この量 | **×** | ○ | ○ |
 *
 * ## その「倍率の基準」は、素材の中からは持ってこられない（2026-09-18・3 回目に測って打ち切り）
 *
 * 残っていた道（発話の区間が分かってから、その区間の声の大きさを基準にする＝2 段階）を測った。
 * **塞がっている。** 正解の発話区間から作る「理想の基準」でも:
 *
 * | | 基準が作れた本数 |
 * | --- | --- |
 * | 声あり | **15 / 15** |
 * | 声なし | **0 / 11** |
 *
 * 声の無い素材に「声の大きさ」は無いので当然だが、
 * **素材単位の判定が答えを出したいのはその声なしの側**である。
 * **分母が要る側では分母が作れず、分母が作れる側ではもう答えが出ている。**
 * 1 段目の精度をどれだけ上げても届かない（正解を渡しても届かないので）。
 *
 * 「その素材の深さの最大」を基準にすると性質は初めて全部そろう
 * （倍率に不変・底に不変・大きさを持つ・声なしでも作れる。検算 ⑨）。
 * **それでも駄目なのは、群を分けないどころか、並ぶ順が「声があるか」ですらないから。**
 * 中央のコマが最大からどれだけ離れているかで並べると、離れている側は**黙る時間のある素材**、
 * 近い側は**鳴りっぱなしの素材**で、音楽はその間に散る
 * （声あり -13.47〜-0.49dB / 声なし -5.10〜-1.31dB）。**測っているのは「鳴りっぱなしか」。**
 * コマ単位で声なしの素材まで混ぜると、線 -3dB で音楽のコマの 52.7% が「声」の側に来る。
 *
 * **3 通りの外れ方は同じ 1 行にまとまる: 素材の中から持ってきた基準は、
 * 声が無ければ背景から作られる。** 素材の中に「声ならこれくらいの大きさだ」と言うものが無い。
 * 残るのは**素材の外から**（配信の音量基準。ショート動画はだいたい -14〜-16 LUFS）だが、
 * それは**入力への仮定**であって素材から測れるものではない。入れるなら仮定と明記すること。
 *
 * ## コマ単位では、この量のほうがはっきり強い（2026-09-18・3 回目）
 *
 * 素材単位では上のとおり使えないが、**コマ単位で声と背景を分ける力は既定の量より強い。**
 * 理由は `modulationDepthDb` の注の裏返しで、**dB は小さな打点の「10 倍」と
 * 声の「10 倍」を同じ 10dB として数える**（打点は合間に底へ落ちるので深く見える）。
 *
 * | 素材 | 既定（dB）の AUC | この量の AUC |
 * | --- | --- | --- |
 * | `speech-drums` | **0.430（偶然以下）** | 0.952 |
 * | `speech-vowels-hats` | 0.632 | 0.966 |
 * | `speech-sustained-hats` | 0.622 | 0.901 |
 *
 * 「その素材の深さの最大から -6dB」を門にすると、いまのコマ単位の判定を 3 つとも上回る
 * （残せた 95.4% / うち声 84.3% / 落とした背景 78.2% 対 94.4% / 73.1% / 57.4%）。
 *
 * **それでも入れていない。** 打点を 2 倍にすると低い側が合間にへこみ、
 * `lowBandReadable` が読めるコマを返さなくなる（`speech-sparse-thump-loud.wav` で 12.38s → 0.04s）。
 * **門は誤るのではなく、自分から働かなくなる。** 先に直すのは `lowBandReadable` のしきい値。
 */
