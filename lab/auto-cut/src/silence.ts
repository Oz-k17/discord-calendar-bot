...
export function planJetCut(
  ...
  // **この門で素材単位に止められるのは、声のある素材だけ**という向きになっている。
  // つまり門が立てる「声が見つからない」は、構造上どれも誤りのほうへ倒れる。
  //
  // 代価も測ってある。取り違えて止まるのは間違いのコストが最大（道具が丸ごと働かなくなる）で、
  // 門が稼ぐのは割合の精度だけ。釣り合わないので、ここは門を捨てる側に倒す。
  //
