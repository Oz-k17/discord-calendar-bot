...
export const REFRAME_FIXTURES = [
  ...
  {
    /**
     * **2 秒で行って戻る**被写体（2026-09-25・2 回目）。
     *
     * 足したのは「**ならしの窓を広げる**」という手を潰すため。
     * 生の位置を中央値でならしているので、**窓より短い動きは窓の中で少数派になって消える。**
     * 寄り道は **2 回 × 0.8 秒**——既定の窓（1.0 秒）なら 8 割を占めて中央値に残り、
     * 2.0 秒へ広げると 4 割になって消える、**その境目をまたぐ長さ**にしてある。
     *
     * ここまでの素材は「ずっと動く」か「ずっと止まる」かのどちらかで、
     * **どれもならしを広げても損をしない形**だった。それだけで広げる/広げないを決めると、
     * 「自分に都合のいい素材で測っただけ」になる。
     *
     * 置き所（0.25）を真ん中から外してあるのは、**「ずっと真ん中」が満点になってしまわない**ため。
     * 寄り道の幅（0.22）は `maxSpeed`（0.22/秒）で届く大きさ——届かない幅にすると、
     * **ならしのせいで追えないのか速さの上限で追えないのかが混ざる。**
     * 人がものを取りに横へ寄って戻る、という短尺の動画ではありふれた動き。
     *
     * 種が 126 なのは、**125 だけ背景の列が被写体より浮いたから**（12 通り試した）。
     * 11 通りは寄り道中の生の位置が 0.457〜0.479（正解 0.47）を指すのに、
     * **125 だけ 0.887** と、被写体ではなく背景を指す。
     * **測りたいのはならしの窓なので、指す所が外れる素材を混ぜると読めなくなる**ため避けた。
     * 背景が被写体より浮くこと自体は `spatial` の弱点として残っている（この回の題材ではない）。
     */
    name: 'subject-dart',
    note: '被写体が 2 秒ほど横へ寄って、すぐ元へ戻る。カットは 1 つも無い',
    hard: true,
    cuts: [],
    options: {
      seed: 126,
